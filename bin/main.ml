...
let make_in_channel (file_path : string) : in_channel = 
        ...
        with Sys_error x ->
                (
                print_endline x;
                failwith "Error opening file. Exitting.\n"
                )


let () = 
        print_endline "Full path to your file (default is `input.vm'): ";
        ...
        let program_asm = Encode.Program.encode program_ast input_file in

        let h = Hashtbl.create 10 in
        Machine.Program.populate h program_asm;

        let binary_list = Machine.Program.encode_pretty_string h program_asm in
